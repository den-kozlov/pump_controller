...
public:
  ...
  Sensor() : onLevelChange(nullptr) {}
  virtual ~Sensor() = default;
  virtual uint8_t getLevel() = 0;
  virtual void tick() = 0;

  void setOnLevelChange(std::function<void()> callback) { onLevelChange = callback; }
...
public:
  ...
  TriggerSensor(uint8_t pin, uint16_t rattleThreshold = DEFAULT_RATTLE_THRESHOLD, bool activeLow = true)
      : Sensor(), pinNumber(pin), rattleThreshold(rattleThreshold), triggered(false), activeLow(activeLow) {
